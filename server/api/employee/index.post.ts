...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (
      !isValidName(name) ||
      (surname.length > 0 && !isValidName(surname)) ||
      (middlename.length > 0 && !isValidName(middlename))
    ) {
    ...
    const colorIndexPosition = Math.min(positionsCount, POSITION_COLORS.length - 1)
    ...
    const colorIndexEmployee = Math.min(employeesCount, EMPLOYEE_COLORS.length - 1)
